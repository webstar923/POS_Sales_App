...
//VV POS ALTERATION var intelligentScroll = true;
var intelligentScroll = false;
...
function virtualKeyboardChromeExtension_click(key, skip) {
	if (top != self) {
		...
		switch (key) {
			...
			//VV POS ALTERATION START the orig toggle above ('&123') doesn't work properly. this is used to swich keyboard layouts in the delivery field on the checkoud modal
			case '&1234':
				console.log('SDD');
			...
			case '&ABC':
				...
				break;
			//VV POS ALTERATION STOP


			case 'Close': //VV change to "CloseXX" to prevent closing the keyabord - used for development only.
...
function virtualKeyboardChromeExtension_open_part2(pos) {
	...
	if (virtualKeyboardChromeExtensionFullScreenState) {
		if (((document.body.style.marginBottom == "") && (!virtualKeyboardChromeExtensionPagePadding)) || (virtualKeyboardChromeExtensionPagePadding)) {
			...
			//POS ALTERATION
			//VV preventing scrolling bars on first keyboard show
			//VV document.body.style.marginBottom = document.getElementById("virtualKeyboardChromeExtension").offsetHeight+"px";
	...
	if (virtualKeyboardChromeExtensionTouchEvents != undefined) {
		...
		if ((e.tagName == "INPUT") || (e.tagName == "TEXTAREA")) {
			if (e.getAttribute("_vkEnabled") != "true") {
				...
				// GM Avoid to add focus on checkbox, maybe we need to add to other element,
        // GM I think there is a bug, why the VK add event to checkbox... never need the keyboard for that !
        if (e.type != "checkbox") {
				  e.focus(); //VV TODO  - probably not the best way. but it works so far
				}
        //POS ALTERATION - 1) open a deal, untick a default ingredient, scroll down -> the pages jumsp back to up.
			}
		}
		delete e;
	}
