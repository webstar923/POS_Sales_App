...
//        var_dump($results);
        ...
        $delivery_or_table_order = $this->TXT_DOUBLE2_SIZE . $this->TXT_CENTER_JUSTIFICATION . $this->TXT_BOLD_ON . $delivery_or_table_order . $this->TXT_BOLD_OFF . ' ORDER:';
       // $delivery_or_table_order = $this->TXT_DOUBLE2_SIZE . $this->TXT_CENTER_JUSTIFICATION . $delivery_or_table_order . $this->TXT_NORMAL_SIZE . $this->TXT_LEFT_JUSTIFICATION;
        //VV
        //$fh = fopen('/var/www/html/posapp/application/libraries/printer_template/log3.txt', 'w') or die("Can't open file.");
        //$results = print_r($contents, true);
...
//       var_dump($products);
        ...
        {
            $cart_contents .= $this->TXT_LEFT_JUSTIFICATION . $this->LINE_SPACING_1 . $this->TXT_FONT_B . $this->TXT_DOUBLE2_SIZE . $this->TXT_BOLD_ON . strtoupper($product['qty']) . 'x ' . strtoupper($product['name']) . $this->TXT_BOLD_OFF;
        ...
        $price_calculation = $this->TXT_RIGHT_JUSTIFICATION . $this->TXT_DOUBLE2_SIZE . $this->TXT_UNDERL2_ON . 'TOTAL:$' . number_format($total, 2)
        ...
        if (property_exists($contents, 'order_status') && $contents->order_status != '')
        {
            ...
            if($payment_status[0] == 'UNPAID') {$payment_status[1] = 'Parked Order';}
        ...
        if (count($customer) > 0)
        {
            $customer_info = $this->TXT_LEFT_JUSTIFICATION . $this->TXT_DOUBLE2_SIZE . 'DELIVERY DETAILS:' . "\n" . $customer_info . $this->TXT_NORMAL_SIZE;
        } else
        {
            $customer_info = '';
        }
        ...
        $header = $this->TXT_DOUBLE2_SIZE . $this->TXT_CENTER_JUSTIFICATION . $contents->order_code . $this->TXT_NORMAL_SIZE;
       ...
       // sleep(5); //when printing without sleep() the cash drawer (on the second printer?) won't open -  TO BE FIXED
        }
