...
Class Printer
{
    ...
    protected $TXT_LEFT_JUSTIFICATION   = "\x0a \x1b\x61\x00";
    ...
    public function callInvoice($print, $contents)
    {
        ...
        foreach ($print as $printer_id => $copies)
        {
            ...
            for ($i = 0; $i < $copies; $i++)
            {
                $fp = @fsockopen($printer->ip, $printer->port, $errno, $errstr,
                                10);
                if ($fp && is_resource($fp))
                {
                    ...
                    fwrite($fp, $out);
                    fwrite($fp,
                            "\012\012\012\012\012\012\012\012\012\033\151\010\004\001");
                    fclose($fp);
...
//        var_dump($contents);
        ...
        $fh = fopen('/var/www/html/posapp/application/libraries/printer_template/log.txt',
...
//        var_dump($products);
        ...
        foreach ($products as $product)
        {
            ...
            {
                $cart_contents .= $this->format_extra(json_decode($product['extra'],
                                1), $product);
            }

            $cart_contents = str_replace(array('Crust:', 'Size:', 'Sauce:'),
                    array('CRUST:', 'SIZE:', 'SAUCE:'), $cart_contents);
            $cart_contents .= $this->TXT_FONT_B . $this->TXT_DOUBLE2_SIZE . $this->TXT_BOLD_ON . $this->TXT_RIGHT_JUSTIFICATION . ' - $' . $product['tot_price'] . $this->TXT_NORMAL;
            //$cart_contents .=  "\n";
            $total += $product['price'];
        }

        //VV$price_calculation = 'Total: $'.number_format($total, 2);
        $price_calculation = $this->TXT_RIGHT_JUSTIFICATION . $this->TXT_DOUBLE_SIZE . $this->TXT_UNDERL2_ON . 'Total:$' . number_format($total,
                        2)
        ...
        {
            $price_calculation .= "\n\n" . $this->line($contents->order_status,
                            'center');
        }
...
//            var_dump ($allDataAboutOrder["extra_cart"] );
            ...
            $positionEntry = strpos($allDataAboutOrder["extra_cart"],
                    SEARCHED_WORD);
            $positionEnd   = strpos($allDataAboutOrder["extra_cart"],
                    '</i><br/>');
            $length        = $positionEnd - $positionEntry;

            // taking necessary start of line
            $nameOfFirstPartOfPizza = substr(substr($allDataAboutOrder["extra_cart"],
                            strlen(SEARCHED_WORD)), $positionEntry, $length);

            // cutting off the end of the line 
            $nameOfFirstPartOfPizza = substr($nameOfFirstPartOfPizza, 0,
                    strlen($nameOfFirstPartOfPizza) - strlen('</i><br/>'));
...
//            var_dump($nameOfPizza);
            ...
            if ($secondPart[0] != '')
            {
                ...
                $secondPart[0] = "\n" . $space . '   > NO:' . $trimmedName;
            }

            if ($secondPart[1] != '')
            {
                $trimmedName   = $this->trimLastComa($secondPart[1]);
                $secondPart[1] = "\n" . $space . '   > EXTRA:' . $trimmedName;
            }

            $nameOfFirstPartOfPizza = $nameOfPizza . $secondPart[0] . $secondPart[1];
